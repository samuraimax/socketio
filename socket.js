...
module.exports = function(server, localhost) {
  ...
  var postUser = function(callback) {
    request.post({
      ...
      form: {
        username: 'socket@ving.co.th',
        password: 'Ltl5MvYYpMmzdOvbtSO5xA==',
        deviceToken: 'socket',
        platform: 'web',
        keyPush: 'socket'
      }
            ...
            clientRedis.llen('video:message:' + socket.video, function(err, length) {
              clientRedis.lrange('video:message:' + socket.video, length - 20, length, function(err, message) {
                ...
                result.forEach(function(value) {
                  ...
                  member.push({user_id: temp_data.id, displayName: temp_data.displayName, profile_pic: temp_data.profile_pic, socket_id: socket.id});
                ...
                message.forEach(function(value) {
                  ...
                  history.push({user_id: temp_data.user.id, displayName: temp_data.user.displayName, profile_pic: temp_data.user.profile_pic, message: temp_data.message, created_at: temp_data.created_at});
  ...
  function taskUserAdmin() {
    ...
    jobs.create('user', {}).delay(1000).priority('high').removeOnComplete(true).save();
  ...
  jobs.process('user', 1, function(job, done) {
    postUser(function(response, result) {
      ...
      if (response !== null) {
        ...
        jobs.create('video', {accessToken: userAdminToken}).delay(1000).priority('high').removeOnComplete(true).save();
      }
  ...
  function create(message, video, member) {
    ...
    job.on('complete', function() {
      var d = new Date();
      var created_at = Math.floor(d.getTime() / 1000);
      clientRedis.rpush('video:message:' + video, JSON.stringify({user: member, message: message, created_at: created_at}), function(err, result) {
        if (err) {
          console.log(err);
          return err;
        }
